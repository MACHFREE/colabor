 ...
 * Class for MyObject
 */
class colabor extends CommonObject
{
	/**
	 * @var string ID to identify managed object
	 */
	public $element = 'colabor';

	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'colabor';
	...
	public function fetch($id)
	{
		...
		$sql = "SELECT rowid, ref, fk_user_create, fk_user_modif, fk_element, fk_ref, elementtype,";
		$sql .= " description, descriptionedit, datec, datee, state,entity";
		$sql .= " FROM ".MAIN_DB_PREFIX."colabor";
       	$sql .= " WHERE rowid=".$id;
        $sql .= " AND state = 1 and entity= ".$conf->entity." ORDER BY datec desc";

		dol_syslog(get_class($this)."::fetchjob sql=".$sql);
		...
		if ($resql) 
		{
          	if ($this->db->num_rows($resql)) 
            {
				...
				$this->fk_ref 				= $obj->fk_ref;
				$this->elementtype			= $obj->elementtype;
				$this->description			= $obj->description;
				$this->descriptionedit		= $obj->descriptionedit;
 ...
 */
	...
	public function get_tipo_anotaciones($rowid,$list =0)
	{
		global $langs, $db, $conf;

		$sql  = "SELECT rowid,label as label FROM " . MAIN_DB_PREFIX . "colabor_type_annotation";
		$sql .= " WHERE active=1";
		if($rowid>0)$sql .= " and rowid=" . $rowid;
		...
		if ($resql) {
			if($list==0){
				...
				while ($i < $num) {
					...
					$labeltoshow =dol_trunc($obj->label, 45);
					$optionsToReturn[$obj->rowid] =  $labeltoshow;
					$i++;
				}
			}elseif($rowid>0){
				$obj = $db->fetch_object($resql);
				$optionsToReturn = $obj->label;
			}
