 ...
 */
class colabor_box_project_2 extends ModeleBoxes
{
	/**
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = "Colabor Toma de Huellas";
	...
	public function loadBox($max = 50)
	{
		...
		$text = $langs->trans("filefingerprint", $max);
		...
		if($user->rights->colabor->read){
			...
			if ($result) {
				...
				while ($i < $num) {
					...
					$service_search   = 'ADM.EXT.4.05';//cadena del servicio con el que se filtrará los projectos
					$services_in = strpos($product->label, $service_search);
					...
					if ($services_in !== false) {
						...
						$this->info_box_contents[$i][2] = array('td' => 'align="left"',
							'text' => $societe->tva_intra);

						$this->info_box_contents[$i][3] = array('td' => 'align="left"',
							'text' => $societe->country);

						$datenac = $societe->array_options ["options_datenac"];
						$this->info_box_contents[$i][4] = array('td' => 'align="left"',
							'text' => dol_print_date($datenac, 'day', ''));
						...
						$this->info_box_contents[$i][6] = array('td' => 'align="left"',
							'text' => '',
							'url' => $user->getNomUrl(-1, '', 0, 0, 16, 0, 'firstelselast', '').'<a');

						$this->info_box_contents[$i][7] = array('td' => 'align="left"',
							'text' => img_edit_add(),
						...
						$this->info_box_contents[$i][8] = array('td' => 'align="left"',
							'text' => img_edit(),
							'url' => DOL_URL_ROOT."/projet/card.php?id=".$project->id);
					}
